...
def match_dim_distribution(mastitis_group, healthy_group, random_state=None, window_start=None, window_end=None):
    ...
    if window_start is not None and window_end is not None:
        ...
        healthy_cow_ids = healthy_group['cow_id'].unique()
        suitable_healthy_cows = []
        
        for cow_id in healthy_cow_ids:
            cow_data = healthy_group[healthy_group['cow_id'] == cow_id]
        ...
        if len(suitable_healthy_cows) > 0:
            ...
            n_mastitis_cows = len(mastitis_group['cow_id'].unique())
...
def create_enhanced_cow_sequences_for_two_stage(mastitis_data, healthy_data, spectral_cols=None, milk_composition_cols=None, 
    ...
    for cow_id in mastitis_data['cow_id'].unique():
        cow_data = mastitis_data[mastitis_data['cow_id'] == cow_id].copy()
        cow_data = cow_data.sort_values('milksampledate')
    ...
    for cow_id in mastitis_data['cow_id'].unique():
        ...
        if len(cow_data) >= 1:
            ...
            existing_dates = cow_data['milksampledate'].values
    ...
    for cow_id, seq_data in mastitis_cow_sequences.items():
        ...
        if real_indices.any():
            ...
            cow_data = mastitis_data[mastitis_data['cow_id'] == cow_id]
    ...
    # Find healthy cows with consecutive DIM values
    healthy_cow_ids = healthy_data['cow_id'].unique()
    suitable_healthy_cows = []
    
    for cow_id in healthy_cow_ids:
        cow_data = healthy_data[healthy_data['cow_id'] == cow_id]
        cow_dims = sorted(cow_data['dim'].values)
    ...
    if len(suitable_healthy_cows) > 0:
        ...
        for dim_value, count in mastitis_dim_counts.items():
            ...
            healthy_cow_ids_with_dim = healthy_with_dim['cow_id'].unique()
            ...
            for cow_id in selected_cow_ids:
                cow_data = healthy_data[healthy_data['cow_id'] == cow_id].copy()
                cow_data = cow_data.sort_values('milksampledate')
    ...
    if matched_healthy_cows:
        ...
        for cow_data in matched_healthy_cows:
            ...
            cow_dates = cow_data['milksampledate'].values
...
def prepare_enhanced_features(data, spectral_cols, milk_composition_cols, scaler_type='minmax', 
    ...
    features_to_fill = ['milkweightlbs', 'cells']
    ...
    parity_encoded = np.array([encode_parity(p) for p in data_filled['parity']])
    
    # 4. Combine metadata features
    metadata_features = np.column_stack([
        data_filled['dim'].values.reshape(-1, 1),           # DIM
        data_filled['milkweightlbs'].values.reshape(-1, 1), # Milk weight (filled)
        data_filled['cells'].values.reshape(-1, 1),         # Somatic cell count (filled)
    ...
    print(f"Metadata dimension: {metadata_scaled.shape[1]} (DIM(1) + MilkWeight(1) + Cells(1) + Parity(3))")
...
def print_enhanced_summary(all_results, model_info=None):
    ...
    composition_results = {}
    composition_results.update(all_results.get('two_stage_composition_transformer', {}))
...
def modeling(time_window=5, step=1, num_epochs=100, input_file='Data/ft-mir-pathogen_neg.csv'):
    ...
    Args:
        ...
        num_epochs (int): Number of training epochs, default 15
    """
    ...
    # Get spectral columns (2800-3000 cm⁻¹)
    spectral_cols = []
    for col in df.columns:
        try:
            ...
            if col_clean.replace('.', '').isdigit() or '.' in col_clean:
                ...
                # if 2800 <= wavelength <= 3000:
                if (1000 <= wavelength <= 3000 
                    and not (1800 <= wavelength <= 2800) 
                    and not (1585 <= wavelength <= 1700)):
                    spectral_cols.append(col)
        except:
            continue
    
    # Define milk composition columns
    milk_composition_cols = [
        "totalfa", "protein", "lactose", "mun", 
        "denovofa", "mixedfa", "preformedfa", "dnrel", "mixrel", "pfrel", 
        "c160", "c180", "c181c9", "nefa", "bhb", "acetone"
    ]
    
    # Define columns containing only 3 main components
    milk_composition_3_cols = ["totalfa", "protein", "lactose"]
    
    # Check which milk composition columns exist
    available_composition_cols = [col for col in milk_composition_cols if col in df.columns]
    print(f"Available milk composition features: {len(available_composition_cols)}/{len(milk_composition_cols)}")
    print(f"Spectral feature count: {len(spectral_cols)}")
    
    # Separate data
    mastitis_data = df[df['group'] == 'mastitis'].copy()
    healthy_data = df[df['group'] == 'healthy'].copy()
    ...
    data_distributions = {}
    detailed_metrics = {}
    training_losses = {}  # New: store training loss
    model_info = {}  # New: store model size and running time information
    ...
    for strategy_name, config in strategies.items():
        ...
        training_losses[strategy_name] = {}  # New: strategy-level loss records
                ...
                for iteration in range(20):
                    try:
                        ...
                        if config['features'] == 'spectral':
                            ...
                            comp_cols = None
                        elif config['features'] == 'composition':
                            feature_cols = None
                            comp_cols = available_composition_cols
            ...
            for window_start, window_end in windows:
                ...
                for iteration in range(20):
                    try:
                        ...
                        all_cow_ids = list(window_mastitis['cow_id'].unique()) + list(healthy_data['cow_id'].unique())
                        print(f"    Total cows: {len(all_cow_ids)}")
                        print(f"    Mastitis cows: {len(window_mastitis['cow_id'].unique())}")
                        print(f"    Healthy cows: {len(healthy_data['cow_id'].unique())}")
                        ...
                        for fold_idx, (train_cow_idx, test_cow_idx) in enumerate(kfold.split(all_cow_ids)):
                            ...
                            train_mastitis = window_mastitis[window_mastitis['cow_id'].isin(train_cow_ids)].copy()
                            test_mastitis = window_mastitis[window_mastitis['cow_id'].isin(test_cow_ids)].copy()
                            
                            train_healthy = healthy_data[healthy_data['cow_id'].isin(train_cow_ids)].copy()
                            test_healthy = healthy_data[healthy_data['cow_id'].isin(test_cow_ids)].copy()
    ...
    save_results_to_csv(all_results, data_distributions, detailed_metrics, training_losses, time_window, step)
    
    # 6. Output summary
    print_enhanced_summary(all_results, model_info)
    
    return all_results, data_distributions, detailed_metrics
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_file', type=str, default='Data/ft-mir.csv',
                       help='Input data file path (default: Data/ft-mir.csv)')
